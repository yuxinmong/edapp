package com.ed.v1.common.widget.wheel;

import java.util.ArrayList;

import com.ed.v1.R;
import com.ed.v1.util.DensityUtils;
...
public class WheelView3 extends View {
	 ...
	 * 默认字体
	 */
	private float normalFont = 90.0f;
	/**
	 * 选中的时候字体
	 */
	private float selectedFont = 110.0f;
	/**
	 * 单元格高度
	 */
	 ...
	 * 默认字体颜色
	 */
	private int normalColor = 0x4cffffff;
	/**
	 * 选中时候的字体颜色
	 */
	...
	public WheelView3(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		unitHeight = (int) attribute.getDimension(
				R.styleable.WheelView_unitHight, 70);
		normalFont = attribute.getDimension(
				R.styleable.WheelView_normalTextSize, 90.0f);
		selectedFont = attribute.getDimension(
				R.styleable.WheelView_selectedTextSize, 110.0f);
		itemNumber = attribute.getInt(R.styleable.WheelView_itemNumber, 5);
		normalColor = attribute.getColor(R.styleable.WheelView_normalTextColor,
				0x4cffffff);
		selectedColor = attribute.getColor(
				R.styleable.WheelView_selectedTextColor, 0xffffffff);
		lineColor = attribute.getColor(R.styleable.WheelView_lineColor,
				0xff000000);
		lineWidth = attribute
				.getDimension(R.styleable.WheelView_lineHeight, 2f);
	...
	private class ItemObject {
		...
		public void drawSelf(Canvas canvas) {
			...
			if (isSelected()) {
				...
				textPaint.setShadowLayer(1.5f, 0, 1f, 0xff3368bc);
				// 获取距离标准位置的距离
				...
				float textSize = normalFont
						+ ((selectedFont - normalFont) * (1.0f - moveToSelect
								/ unitHeight));
				textPaint.setTextSize(DensityUtils.sp2px(getContext(), 55));
			} else {
				if (id == getSelected() - 1 || id == getSelected() + 1) {
					textPaint.setTextSize(DensityUtils.sp2px(getContext(), 45));
					textPaint.setColor(Color.parseColor("#4cffffff"));
					textPaint.setShadowLayer(0f, 0, 0f, 0x00000000);
				} else {
					textPaint.setTextSize(DensityUtils.sp2px(getContext(), 30));
					textPaint.setColor(Color.parseColor("#19ffffff"));
					textPaint.setShadowLayer(0f, 0, 0f, 0x00000000);
				}
			...
			canvas.drawText(
					itemText,
					x - textRect.width() / 2
							+ DensityUtils.dip2px(getContext(), 40), y + move
			...
			Log.i("1244",
					x
							...
							+ (textRect.width() / 2)
							+ "dev:"
							+ (DensityUtils.dip2px(getContext(), 40))
							+ "tota"
							+ (x + controlWidth / 2 - textRect.width() / 2 + DensityUtils
									.dip2px(getContext(), 40)));
		}
